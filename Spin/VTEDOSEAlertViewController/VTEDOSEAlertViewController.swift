...
class VTEDOSEAlertViewController: UIViewController {
    // MARK: Properties
    ...
    @objc fileprivate func loadNibAlertController() -> [AnyObject]?{
        ...
        if let bundleURL = podBundle.url(forResource: "VTEDOSEAlertViewController", withExtension: "bundle"){
            
            if let bundle = Bundle(url: bundleURL) {
                return bundle.loadNibNamed("VTEDOSEAlertViewController", owner: self, options: nil) as [AnyObject]?
            }
            else {
